...
class fox:
    ...
    def eat(self, rabbits):
        ...
        #print("Fox eating - nearest rabbit is {0} away".format(distance) )
        #print(nearest_rabbit_ind)
        if distance <= self.speed and nearest_rabbit_ind is not -1:
             pk = 1 - (distance / self.speed)
             #print(distance)
             #print(rabbits[nearest_rabbit_ind])
             if pk > rand():
                ...
                #print("Rabbit at {0} got nommed".format(rabbits[nearest_rabbit_ind].old_pos))
    ...
    def migrate(self, env):
        ...
        direc = rand()*2*np.pi
        npos = np.zeros(2)
        #print('initial direction is {0}'.format(direc))
...
class rabbit:
    ...
    foodbrd =  10             # minimum food threshold for breeding
    maxage = 50               # maximum age allowed
    dead = False                # Is this agent dead or alive from age or starvation
    has_been_eaten = False     # Has this agent been eaten?
    ...
    def __init__(self, age=[], food=[], pos=[], speed=[], last_breed=[]): 
        ...
        self.pos = pos
        self.old_pos = pos #Temporary, to emulate behaviour of MATLAB original
        self.cpos = np.round(pos).astype(np.int) - 1  # round up position to nearest grid point
        self.speed = speed
        self.last_breed = last_breed
    def __repr__(self):
        out = 'Age : {0}\nFood : {1}\nPos : {2}\nSpeed: {3}\nlast_breed: {4}\n'.format(self.age,self.food,self.pos,self.speed,self.last_breed)
    ...
    def breed(self):
        ...
        if self.food >= self.foodbrd and self.last_breed >= self.brdfq:
            ...
            self.food = self.food/2.0
            self.last_breed = 0
            #print('Rabbit at {0} bred'.format(self.pos))
        else:
            self.last_breed = self.last_breed + 1
        self.age = self.age + 1
        return(new)
    ...
    def die(self):
        # rabbit.die(self)
        # rabbits die if their food level reaches zero or they are older than maxage
        if self.food <= self.minfood or self.age > self.maxage:
            self.dead = True
            #print('Rabbit Died')
    
    def eat(self, env):
        pfood = env.food[self.cpos[0], self.cpos[1]]  # obtain environment food level at current location
        if pfood >= 1:  # if food exists at this location
            env.food[self.cpos[0], self.cpos[1]] = env.food[self.cpos[0], self.cpos[1]] - 1
            self.food = self.food + 1
            eaten = 1
        else:
            self.food = self.food - 1
    ...
    def migrate(self, env):
        ...
        if not(mig):  # rabbit has been unable to find food,  so chooses a random direction to move in
            cnt = 1
            dir = rand()*2*np.pi
            while not(mig) and cnt <= 8:
                npos[0] = self.pos[0] + self.speed * np.cos(dir)
                npos[1] = self.pos[1] + self.speed * np.sin(dir)
                if npos[0] < env.bm_size and npos[1] < env.bm_size and npos[0] >= 1 and npos[1] >= 1:   # check that fox has not left edge of model - correct if so.
                    mig = True
                cnt = cnt + 1 
                dir = dir + (np.pi/4)
